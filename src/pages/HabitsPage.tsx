...
export default function HabitsPage() {
  ...
  async function addHabit(e: React.FormEvent) {
    ...
    setHabits((prev) => [
      { ...data, weekly_log: { [today]: false } } as Habit,
      ...prev,
    ]);
    setNewHabitName('');
  ...
  async function toggleHabit(id: string, completedToday: boolean, streak: number, weeklyLog: Record<string, boolean>) {
    const today = new Date().toISOString().split('T')[0];
    const newStreak = completedToday ? Math.max(0, streak - 1) : streak + 1;
    const newLog = { ...weeklyLog, [today]: !completedToday };
    const { error } = await supabase
      .from('habits')
      .update({
        completed_today: !completedToday,
        streak: newStreak,
        last_completed: !completedToday ? today : null,
        weekly_log: newLog,
      })
      .eq('id', id);
    if (error) {
    ...
    setHabits((prev) =>
      prev.map((h) =>
        h.id === id
          ? { ...h, completed_today: !completedToday, streak: newStreak, weekly_log: newLog }
          : h
      )
    );
  }
  ...
  const avgCompletion = totalHabits > 0
    ? Math.round((habits.filter((h) => h.completed_today).length / totalHabits) * 100)
    : 0;
          ...
          <div className="space-y-3">
            {habits.map((habit, index) => (
              ...
              >
                {/* Check-in */}
                <button
                  onClick={() => toggleHabit(habit.id, habit.completed_today, habit.streak, habit.weekly_log)}
                  className="shrink-0"
                >
